...
var _delegate3, _ProductCard_instances, onSwatchHovered_fn, onSwatchChanged_fn, createMediaImg_fn, createInputImage_fn;
...
onSwatchHovered_fn = async function(event, target) {
  ...
  } else if (control.hasAttribute("data-non-variant-image-url")) {
    __privateMethod(this, _ProductCard_instances, createInputImage_fn).call(this, control, primaryMediaElement.className, primaryMediaElement.sizes);
  }
};
onSwatchChanged_fn = async function(event, target) {
  ...
  if (!target.hasAttribute("data-variant-media") && !target.hasAttribute("data-non-variant-image-url")) {
    return;
  }
  let primaryMediaElement = this.querySelector(".product-card__image--primary"), secondaryMediaElement = this.querySelector(".product-card__image--secondary"), newPrimaryMediaElement = null, newSecondaryMediaElement = null;
  if (target.hasAttribute("data-variant-media")) {
    ...
    newPrimaryMediaElement = __privateMethod(this, _ProductCard_instances, createInputImage_fn).call(this, target, primaryMediaElement.className, primaryMediaElement.sizes);
  }
...
createInputImage_fn = function(control, className, sizes) {
  const image = new Image();
  image.className = className;
  image.sizes = sizes || "";
  image.draggable = false;
  image.alt = control.getAttribute("data-non-variant-image-alt") || control.value || "";
  image.src = control.getAttribute("data-non-variant-image-url");
  return image;
};
...
(() => {
  const PICKER_SELECTOR = '[data-lc-satin-stitch-picker="quick-buy"]';
  const SWATCH_SELECTOR = ".lc-satin-stitch-picker__swatch";

  const normalise = (text) => (text || "").toLowerCase().replace(/-/g, " ").replace(/\u00a0/g, " ").replace(/\s+/g, " ").trim();

  function getScope(picker) {
    return picker.closest(".quick-buy-modal__content") || picker.closest("quick-buy-modal") || picker.closest("product-rerender") || document;
  }
  ...
  function getSelectedStyle(picker) {
    const scope = getScope(picker);
    const legends = scope.querySelectorAll(".variant-picker__option legend, variant-picker legend");

    for (const legend of legends) {
      const legendText = normalise(legend.textContent);

      if (legendText === "style" || legendText.startsWith("style:")) {
        const optionRoot = legend.closest(".variant-picker__option") || legend.parentElement;
        const checked = optionRoot?.querySelector('input[type="radio"][data-option-position]:checked');

        return getLabelText(checked);
      }
    }
  ...
  function getHiddenInput(picker) {
    return picker.querySelector('input[name="properties[Satin Stitch Colour]"]');
  }

  function clearPicker(picker) {
    const input = getHiddenInput(picker);
    const selectedLabel = picker.querySelector(".js-selected-stitch-label");
    ...
    picker.querySelectorAll(SWATCH_SELECTOR).forEach((swatch) => {
      swatch.classList.remove("is-active", "is-selected");
    });
  }

  function selectSwatch(picker, swatch) {
    if (!swatch) {
      return;
    }

    picker.querySelectorAll(SWATCH_SELECTOR).forEach((item) => {
      item.classList.remove("is-active", "is-selected");
    });

    swatch.classList.add("is-active", "is-selected");

    const stitchLabel = (swatch.getAttribute("data-stitch-label") || "").trim();
    const input = getHiddenInput(picker);
    const selectedLabel = picker.querySelector(".js-selected-stitch-label");

    if (input) {
      input.value = stitchLabel;
      input.disabled = !stitchLabel;
    }

    if (selectedLabel) {
      selectedLabel.textContent = stitchLabel ? ` - ${stitchLabel}` : "";
    }
  }

  function ensureSwatchSelected(picker) {
    const selected = picker.querySelector(`${SWATCH_SELECTOR}.is-selected, ${SWATCH_SELECTOR}.is-active`);

    if (selected) {
      selectSwatch(picker, selected);
      return;
    }

    selectSwatch(picker, picker.querySelector(SWATCH_SELECTOR));
  }

  function updatePicker(picker) {
    if (!picker || !document.documentElement.contains(picker)) {
      return;
    }

    const style = normalise(getSelectedStyle(picker));
    const hasSwatches = picker.getAttribute("data-has-swatches") !== "false" && Boolean(picker.querySelector(SWATCH_SELECTOR));
    const requiresSatin = style.includes("one row satin stitch") || style.includes("1 row satin stitch") || style.includes("two row satin stitch") || style.includes("2 row satin stitch");
    const noSatin = style.includes("no satin stitch");

    if (hasSwatches && requiresSatin && !noSatin) {
      picker.style.setProperty("display", "block", "important");
      ensureSwatchSelected(picker);
    } else {
  ...
  function initPicker(picker) {
    if (!picker || picker.dataset.lcQuickBuySatinReady === "true") {
      updatePicker(picker);
      return;
    }

    picker.dataset.lcQuickBuySatinReady = "true";
  ...
  function initPickers(root = document) {
    if (root.nodeType === Node.ELEMENT_NODE && root.matches(PICKER_SELECTOR)) {
      initPicker(root);
    }

    if (root.querySelectorAll) {
      root.querySelectorAll(PICKER_SELECTOR).forEach(initPicker);
    }
  }

  function scheduleInit(root = document) {
    window.setTimeout(() => initPickers(root), 80);
    window.setTimeout(() => initPickers(root), 220);
  }

  document.addEventListener("change", (event) => {
    if (event.target?.matches('input[type="radio"][data-option-position]')) {
      scheduleInit(event.target.closest(".quick-buy-modal__content") || document);
    }
  });

  ["variant:change", "variant:changed", "product:rerender"].forEach((eventName) => {
    document.addEventListener(eventName, (event) => scheduleInit(event.target instanceof Element ? event.target : document), true);
  });
